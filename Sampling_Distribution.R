...
#Population
#Observations on the ‘mpg’ variable in ‘mtcars’ data set are taken as population values.

x<-mtcars$mpg

#Generating all possible random samples
#From the above vector of values a random sample of size 4 with replacement is generated using the sample() function in R.

sample<-sample(x,size=4, replace = TRUE)
sample

#Now, we need more samples from this population to plot the sampling distribution of the statistic under 
#study. replicate() function is used to generate some 1000 samples from this population
...
#A plot of the sampling distribution of the sample mean is obtained using

hist(sample_means,main="sample  means")

#The sampling distribution of the sample range is obtained as follows
